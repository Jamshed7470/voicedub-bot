...
| `/review`   | ссылка на студию проверки для последней задачи                                     |
| `/approve`  | утвердить задачу без открытия студии                                               |
| `/speakers` | `/speakers 6` — искать ровно 6 голосов; `/speakers auto` — сбросить                |
| `/mode`     | `clone` / `preset` / `auto` — откуда брать голоса                                  |
| `/voices`   | список голосов в банке                                                             |

## Студия проверки голосов

Перед озвучкой бот останавливается и показывает, кого он услышал и каким
голосом собирается озвучить. Пять минут проверки экономят часы рендера
впустую — особенно на полнометражном фильме, где реплик больше тысячи.

**Зачем это появилось.** В первой версии голос спикера пересчитывался
почти на каждой реплике, и один актёр к концу фильма звучал тремя-четырьмя
разными голосами. Теперь голос каждого человека фиксируется один раз и не
меняется до конца фильма, а перед озвучкой можно глазами проверить, что
система не разделила одного актёра на нескольких.

Включается тремя строками в `.env`:

```
STUDIO_ENABLED=true
STUDIO_SECRET=<длинная случайная строка>
STUDIO_PUBLIC_URL=http://localhost:8080
```

Секрет генерируется командой:

```powershell
.venv\Scripts\python.exe -c "import secrets; print(secrets.token_urlsafe(48))"
```

Запуск во втором окне PowerShell:

```powershell
.venv\Scripts\python.exe -m studio
```

Дальше бот сам пришлёт ссылку. **Подробная пошаговая инструкция —
[docs/STUDIO.md](docs/STUDIO.md).**

Не хотите проверять вручную? `/mode auto` и настройка «Утверждать
автоматически» пропускают паузу: бот озвучит сразу, а ссылку на студию
пришлёт вместе с результатом.

### Банк голосов

Спикерам, у которых меньше 8 секунд чистой речи, клонировать нечего —
им подбирается голос из банка, причём разным спикерам разные голоса.
Собрать банк:

```powershell
# из встроенных голосов модели XTTS-v2
.venv\Scripts\python.exe -m scripts.build_voice_bank --from-xtts

# или из своих записей (30-60 с чистой речи, имя файла = название голоса)
...
│                   #   review.py — точка проверки, studio_commands.py — команды
├── core/           # пайплайн: ffmpeg, ASR, диаризация, перевод, микс, сборка
├── identity/       # кто говорит: отпечатки голоса, слияние кластеров,
│                   #   стабильные ID спикеров между прогонами
├── voices/         # чем говорить: заблокированные профили, банк, кастинг
├── synth/          # озвучка: движок XTTS, Identity QC, рендер
├── project/        # project.json — модель, хранилище, журнал правок
├── studio/         # веб-интерфейс проверки: FastAPI + собранный React
├── configs/        # config.yaml — все настройки пайплайна
├── docs/           # STUDIO.md — руководство, AUDIT.md, DECISIONS.md
├── data/jobs/      # файлы задач (медиа 7 дней, отчёт 30)
├── tests/          # тесты
...
```
диаризация → Speaker Identity Engine → ОДИН профиль на спикера → синтез → QC
  (кластеры)    (сводит одного человека     (латенты XTTS,        (проверка
                 обратно в одного)           считаются 1 раз)      тембра)
```

Ключевое правило: **один speaker_id = один голос на весь фильм**. Профиль
голоса заблокирован; синтез без него невозможен технически — движок
принимает профиль, а не путь к звуковому файлу.
