...
- Длительность — **любая**. Ограничение снято (`limits.max_duration_minutes: 0`
  в `configs/config.yaml`; поставьте число, если хотите вернуть предел).
  Длинный ролик обрабатывается кусками: Demucs идёт отрезками по 10 минут,
  микс собирается блоками — память не зависит от длительности.
- Результат — до **50 МБ** (лимит отправки ботом); при превышении бот сам
  пережмёт видео.

## Куда попадает готовое видео

Готовый дубляж **всегда** сохраняется в папке `готовые видео/` внутри проекта —
до всякой отправки, в полном качестве, рядом с файлом субтитров:

```
готовые видео/2026-08-11 Название ролика [ru].mp4
готовые видео/2026-08-11 Название ролика [ru].srt
```

В чат бот пришлёт видео, если оно влезает в лимит Telegram (при необходимости
пережав). Если ролик длинный и не влезает даже так — бот пришлёт сообщение с
путём к файлу, а работа на несколько часов не пропадёт.

## Обработка без Telegram

Длинный ролик удобнее прогнать напрямую — результат всё равно ляжет в папку:

```powershell
python -m scripts.run_url "https://youtu.be/XXXXXXXX" --lang ru --voice bank
python -m scripts.run_url "C:\путь\к\видео.mp4" --lang ru
```

Прогресс по этапам пишется в консоль и в `logs/run_url.log`.
